...
CREATE OR REPLACE PROCEDURE Scd2Procedure(sourceTable STRING
  , targetTable STRING
  , keyColumns array
  ...
  , scdActiveEndDate STRING)
RETURNS STRING
  LANGUAGE SCALA
  RUNTIME_VERSION = '2.12'
PACKAGES = ('com.snowflake:snowpark:latest')
HANDLER = 'ScdProcess.run'
AS
$$
import com.snowflake.snowpark.functions.{coalesce, col, dateadd, lit, max, to_date}
...
object ScdProcess {
  ...
  def validate_params(session: com.snowflake.snowpark.Session): (String, Seq[Any]) = {
    ...
    if (this.scdEndDateColumnName.isEmpty || this.scdEndDateColumnName.trim() == "")
      error_message = error_message + " Audit Column (scdEndDateColumnName)  not passed for Target table (" + this.targetTable + ")."
    ...
    println("this.keyColumns.length : " + this.keyColumns.length)
  ...
  def run(session: session: com.snowflake.snowpark.Session
  , sourceTable: String
  , targetTable: String
  ...
  , scdActiveEndDate: String): String = {
    ...
    try {
      ...
      val (validation_error_message, validation_commonColumnArray) = validate_params(session)
      ...
      if (error_message.trim().isEmpty) {
        ...
        var targetColumnOrderArray = session.table(this.targetTable).schema.names
        ...
        var dfTarget = session.sql("select * " + ", hash(" + commonColumnString + ") as HASH_ROW_VALUE from " + this.targetTable
          + " where (" + this.scdEndDateColumnName + " IS NULL OR coalesce(" + this.scdEndDateColumnName + ", '1900-01-01') = coalesce("
          + this.scdActiveEndDate + ", '1901-01-01' ) )")
        ...
        var mergeInsertDict: collection.immutable.Map[String, com.snowflake.snowpark.Column] =
          collection.immutable.Map[String, com.snowflake.snowpark.Column]()
